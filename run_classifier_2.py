...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  feature = InputFeatures(
      ...
      label_ids=label_ids,
      is_real_example=True)
  return feature, tokens, label_ids
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
          ...
          precision = tf_metrics.precision(label_ids, predictions, 13, [1, 2, 3, 4, 5, 6, 7, 8], average="macro")#13 是总label数，括号里是有用的
...
def main(_):
  ...
  if FLAGS.do_eval:
      ...
      with tf.gfile.GFile(output_eval_file, "w") as wf:
  ...
  if FLAGS.do_predict:
    ...
    output_test_file = os.path.join(FLAGS.output_dir, "test_prediction.txt")
    with tf.gfile.GFile(output_test_file, "w") as wf:
