...
TPU_ADDRESS = 'grpc://10.113.252.74:8470'
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if len(labellist) != len(textlist):
      print(labellist,textlist)
  assert len(textlist) == len(textlist)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, logits, trans, pred_ids) = create_model(
        ...
        num_labels, False, dropout_rate=0.5, lstm_size=128)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        def metric_fn(label_ids, pred_ids):
            precision = tf_metrics.precision(label_ids, pred_ids, 13, [1, 2, 3, 4, 5, 6, 7, 8], average="macro")
            recall = tf_metrics.recall(label_ids, pred_ids, 13, [1, 2, 3, 4, 5, 6, 7, 8], average="macro")
            f = tf_metrics.f1(label_ids, pred_ids, 13, [1, 2, 3, 4, 5, 6, 7, 8], average="macro")
...
class NerProcessor(DataProcessor):
  """Processor for the en data NER processing."""
